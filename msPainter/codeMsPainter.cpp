...
void update()
{
	...
	mid_top = {(drawTopLeft.x + ((drawBotRight.x - drawTopLeft.x) / 2)),drawTopLeft.y};
	mid_bot = {mid_top.x, drawBotRight.y};	
	range = {mid_top.x - drawTopLeft.x, drawBotRight.y-drawTopLeft.y};

	dropper = {cum_in_calliou.left+120,cum_in_calliou.top+10};
}
int main(){	
	srand (time(NULL));
	
	update();
	random_draw(dropper,range,drawTopLeft,drawBotRight,300000,true);
	
	random_draw(dropper,range,drawTopLeft,drawBotRight,100000);
...
void restrict_number(pos dBR, pos dTL ,pos &number)
{
		if( number.x > (dBR.x-10)){number.x = dBR.x;}
		if( number.y > (dBR.y-10)){number.y = dBR.y;}
		if( number.x < (dTL.x + 5)){number.x = dTL.x;}
		if( number.y < (dTL.y + 5)){number.y = dTL.y;}
}
void random_draw( pos dropper, pos range, pos dTL, pos dBR, int iterations, bool further_rand)
{
	...
	for(int i =0; i< iterations; i++)
	{
		...
		restrict_number(dBR,dTL,rand_pos1);
		clickMouse(3,20);	
		...
		{
			for(int i=0; i < rand_in_range(30); i++)
			{
				update();
				clickMouse(1);
